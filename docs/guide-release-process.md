...
For React Native (**`>= 0.80`**), a draft Github release is generated by the job [**Publish Release**](https://github.com/facebook/react-native/actions/workflows/publish-release.yml). Verify all details and links are valid and publish it.

#### Verify DevTools Github Release Assets
The Publish Release job uploads several DevTools assets to GitHub Release Assets.

Once the release is published, [Validate DotSlash Artifacts](https://github.com/facebook/react-native/actions/workflows/validate-dotslash-artifacts.yml) will validate that these assets were uploaded correctly. Ensure that this action run is green and that the assets exist on the release:
